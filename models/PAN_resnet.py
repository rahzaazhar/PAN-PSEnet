...
class FPA(nn.Module):
    ...
    def __init__(self,inplanes,planes=512):
        ...
        self.relu_gpb = nn.ReLU(inplace=True)

        # dilated convolution Branchs 
        self.diaconv1 = nn.Conv2d(inplanes,planes,kernel_size=3,stride=1,dilation=3,bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.diaconv2 = nn.Conv2d(inplanes,planes,kernel_size=3,stride=1,dilation=6,bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
        self.diaconv3 = nn.Conv2d(inplanes,planes,kernel_size=3,stride=1,dilation=12,bias=False)
    ...
    def forward(self, x):
        ...
        x_1 = _upsample(x_1,x_master)
        x_2 = _upsample(x_2,x_master)
        x_3 = _upsample(x_3,x_master)

        x_gpb = _upsample(x_gpb,x_master)


        out = torch.cat((x_3,x_2,x_1),1)
...
class GAU(nn.Module):
    ...
    def forward(self, fms_high, fms_low):
        ...
        fms_high_upsample = self.conv1x1(fms_high_upsample)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=7, scale=1):
        ...
        self.conv3 = nn.Conv2d(2048, 256, kernel_size=3, stride=1, padding=1)
