...
def get_new_task_activations(image_loader_500,new_task,train_loaders,val_loaders,datamode,epochs,path_pretrain,n_class=10):
    if datamode == 'pmnist' or datamode == 'smnist' :
        ...
        '''template = {'linear1_input':nn.Linear(32*32,312),'relu1':nn.ReLU(),
                    'linear2':nn.Linear(312,128)}'''
    if datamode == 'CIFAR100':
        ...
        '''template = {'conv2d_input1':nn.Conv2d(3,6,5),'maxpool1':nn.MaxPool2d(2),'relu1':nn.ReLU(),
                    'conv2d_2':nn.Conv2d(6,16,5),'maxpool2':nn.MaxPool2d(2),'relu2':nn.ReLU(),
                    'conv2d_3':nn.Conv2d(16,120,5),'relu3':nn.ReLU(),
                    'adaptavgpool2d':Flatten(),'linear1':nn.Linear(120,84)}'''
    if datamode == 'VDD':
