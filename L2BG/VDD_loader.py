...
def prepare_data_loaders(dataset_names, data_dir, imdb_dir, batch_size, data_usage, shuffle_train=True, index=None):
    ...
    for i in range(len(dataset_names)):
        ...
        if not dataset_names[i] in ['dtd','aircraft','vgg-flowers']:
            print('Data Usage:',data_usage*100,'%')
...
def get_tasks_VDD(dataset_names,data_dir, imdb_dir,batch_size,data_usage):
    ...
    for idx,name in enumerate(dataset_names):
        train_loaders_dict[name] = train_loaders[idx]
        val_loaders_dict[name] = val_loaders[idx]
        num_classes_dict[name] = num_classes[idx]
    return train_loaders_dict, val_loaders_dict, num_classes_dict
