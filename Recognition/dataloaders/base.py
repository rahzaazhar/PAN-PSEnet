...
def get_tasks(train_datasets,val_datasets,batch_size,cuda=False,shuffle=True):
    train_loaders = {}
    val_loaders = {}

    for task in train_datasets.keys():
        train_loaders['task_'+task] = torch.utils.data.DataLoader(train_datasets[task],
                                        batch_size=batch_size, shuffle=True, num_workers=4)
        
        val_loaders['task_'+task] = torch.utils.data.DataLoader(val_datasets[task],
                                        batch_size=batch_size, shuffle=True, num_workers=4)
